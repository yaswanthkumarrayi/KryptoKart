...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    ...
    if (widget.paymentData != null) {
      ...
      _recipientAddress = widget.paymentData!['recipientUpi'] ?? widget.paymentData!['recipientWallet'] ?? '';
      if (widget.paymentData!['amount'] != null && widget.paymentData!['amount'].toString().isNotEmpty) {
  ...
  void _onAmountChanged() async {
    ...
    if (amount > 0 && _isCrypto) {
      try {
        final crypto = await _coinGecko.convertInrToCrypto(amount, _selectedCoin);
  ...
  void _onPaymentSuccess(PaymentSuccessResponse response) async {
    ...
    try {
      ...
      final txnData = await _apiService.createTransaction({
        'txnId': txnId,
        'type': _isCrypto ? 'crypto' : 'upi',
        'amountInr': amount,
        'cryptoCoin': _isCrypto ? _selectedCoin : null,
        'cryptoAmount': _isCrypto ? _cryptoEquivalent : null,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Transaction recorded with issue: $e'), backgroundColor: AppColors.yellow),
        );
      }
    }
  }
  ...
  Future<void> _processPayment() async {
    ...
    if (_isCrypto) {
      ...
      try {
        ...
        if (mounted) {
          setState(() => _isProcessing = false);
          // Fallback: create transaction without Razorpay
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Razorpay error: $e. Creating direct transaction...'),
              backgroundColor: AppColors.yellow,
            ),
          );
          _processDirectPayment(amount);
        }
      }
    }
  }

  Future<void> _processDirectPayment(double amount) async {
    try {
      final txnId = 'TXN-${const Uuid().v4().substring(0, 8).toUpperCase()}';
      final txnData = await _apiService.createTransaction({
        'txnId': txnId,
        ...
        child: Column(
          children: [
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: Stack(
                      children: [
                        const Center(child: Icon(Icons.person, color: AppColors.accent)),
                        Positioned(
                          bottom: 0, right: 0,
                          child: Container(
                            width: 16, height: 16,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(_recipientAddress, style: AppTextStyles.caption, maxLines: 1, overflow: TextOverflow.ellipsis),
                      ],
                    ),
                  ),
                ],
                ...
                children: [
                  Expanded(
                    child: GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          color: _isCrypto ? AppColors.accent : Colors.transparent,
                          borderRadius: BorderRadius.circular(50),
                        ),
                        child: Center(
                          child: Text('Crypto', style: TextStyle(fontWeight: FontWeight.w600, color: _isCrypto ? AppColors.background : AppColors.textSecondary)),
                        ),
                      ),
                    ),
                  ),
                  Expanded(
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Center(
                          child: Text('UPI', style: TextStyle(fontWeight: FontWeight.w600, color: !_isCrypto ? AppColors.background : AppColors.textSecondary)),
                        ),
                      ),
                    ),
                  ),
            ...
            Row(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('₹', style: AppTextStyles.number.copyWith(color: AppColors.textSecondary, fontSize: 28)),
                ...
                SizedBox(
                  ...
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      hintText: '0',
                      hintStyle: TextStyle(color: AppColors.textSecondary, fontSize: 48),
                    ),
                  ),
                ),
              ],
            ...
            GlassCard(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    _isCrypto ? 'BLOCKCHAIN VERIFIED TRANSFER' : 'UPI SECURE PAYMENT VIA RAZORPAY',
